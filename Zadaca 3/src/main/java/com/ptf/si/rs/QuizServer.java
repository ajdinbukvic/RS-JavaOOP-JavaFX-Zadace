...
public class QuizServer {
    ...
    private static Question[] pitanja;

    public static void ucitajPitanja() {
        try {
            String myJsonString = Files.readString(Paths.get("pitanja.json"));
            ObjectMapper om = new ObjectMapper();
            pitanja = om.readValue(myJsonString, Question[].class);
        } catch (Exception ex) {
            System.out.println(ex.getMessage());
        }
    }
    public static void izmjesajPitanja() {
        List<Question> pitanjaList = new ArrayList<>(Arrays.asList(pitanja));
        ...
        pitanja = pitanjaList.stream().limit(10).toList().toArray(new Question[10]);
    ...
    public static class Handler implements Runnable {
        ...
        public boolean provjeriOdgovor(int unosBr) {
            Answer tacanOdgovor = pitanja[this.trenutnoPitanje].getAnswers().stream().filter(Answer::isCorrect).findFirst().get();
